...
# --- 💡 핵심 해결책: 중복 호출을 막기 위한 메모장 추가 ---
if 'current_pic_id' not in st.session_state:
    st.session_state['current_pic_id'] = None # 현재 사진의 고유 번호 기억
if 'temp_title' not in st.session_state:
    st.session_state['temp_title'] = ""       # 제목 메모
if 'temp_year' not in st.session_state:
    st.session_state['temp_year'] = ""        # 년도 메모
# ----------------------------------------------------
...
if picture:
    ...
    # 💡 사진이 '새로' 찍힌 경우에만 제미나이에게 물어보기! (API 중복 호출 완벽 차단)
    if st.session_state['current_pic_id'] != picture.id:
        ...
        with st.spinner('제미나이가 문맥을 분석 중입니다... 🧠'):
            try:
                ...
                3. 출판 년도(예: 1998)가 있다면 4자리 숫자만 추출하세요. 없으면 비워두세요.
    ...
    # 5. 사용자 확인 및 저장 폼 (이제 제미나이 대신 메모장에서 값을 꺼내옴)
